...
function setMetatagsCheevo(cheevo)
{
	g_cheevo = cheevo;
}

function expandMetatags(req) {
  return '    <meta property="og:url" content="http://{{config.host}}/cheevo/'+g_cheevo+'.html"/>'
	  + "\n"
	  + '<meta property="og:title" content="The '+g_cheevo+' Achievement"/>'
	  + "\n";
}
...
function expandSHTML(req, file, filename, options) {
  ...
  for(var i in split) {
    ...
    if((ret = s.match(/{{(.*)}}/))) {
      // expand vars
      result += s.replace(/{{(.*?}})/g, function (m) { return eval(m); });;
    } else if (ret = s.match(/\s*#include\s+[\'\"](.*)[\'\"]/)) {
      var name = ret[1];
    ...
    } else if(ret = s.match(/\s*#metatags/)) {
...
exports.setMetatagsCheevo = setMetatagsCheevo;
